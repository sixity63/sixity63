import { useState, useEffect } from 'react';
import { useNavigate } from 'react-router-dom';
import { supabase } from '@/integrations/supabase/client';
import { toast } from 'sonner';
import { useAuth } from '@/hooks/useAuth';
import { User, Lock, Cpu, ArrowLeft, Mail } from 'lucide-react';

const Auth = () => {
  const navigate = useNavigate();
  const { user } = useAuth();
  const [loading, setLoading] = useState(false);
  const [email, setEmail] = useState('');
  const [password, setPassword] = useState('');
  const [isSignUp, setIsSignUp] = useState(false);
  const [isForgotPassword, setIsForgotPassword] = useState(false);
  const [emailSent, setEmailSent] = useState(false);

  useEffect(() => {
    if (user) {
      navigate('/');
    }
  }, [user, navigate]);

  const handleSignUp = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);

    const { error } = await supabase.auth.signUp({
      email,
      password,
      options: {
        emailRedirectTo: `${window.location.origin}/`
      }
    });

    setLoading(false);

    if (error) {
      toast.error(error.message);
    } else {
      setEmailSent(true);
      toast.success('Link verifikasi telah dikirim ke email Anda!');
    }
  };

  const handleSignIn = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);

    const { error } = await supabase.auth.signInWithPassword({
      email,
      password,
    });

    setLoading(false);

    if (error) {
      if (error.message.includes('Email not confirmed')) {
        toast.error('Email belum diverifikasi. Cek inbox email Anda.');
      } else {
        toast.error(error.message);
      }
    } else {
      toast.success('Login berhasil!');
      navigate('/');
    }
  };

  const handleGoogleSignIn = async () => {
    setLoading(true);
    
    const { error } = await supabase.auth.signInWithOAuth({
      provider: 'google',
      options: {
        redirectTo: `${window.location.origin}/`
      }
    });

    if (error) {
      toast.error(error.message);
      setLoading(false);
    }
  };

  const handleForgotPassword = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);

    const { error } = await supabase.auth.resetPasswordForEmail(email, {
      redirectTo: `${window.location.origin}/reset-password`,
    });

    setLoading(false);

    if (error) {
      toast.error(error.message);
    } else {
      toast.success('Link reset password telah dikirim ke email Anda!');
      setIsForgotPassword(false);
      setEmail('');
    }
  };

  // Email sent confirmation screen
  if (emailSent) {
    return (
      <div className="min-h-screen flex items-center justify-center p-4" style={{ background: '#e0e5ec' }}>
        <div 
          className="w-full max-w-sm p-8 rounded-3xl text-center"
          style={{
            background: '#e0e5ec',
            boxShadow: '20px 20px 60px #bec3c9, -20px -20px 60px #ffffff'
          }}
        >
          <div 
            className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-6"
            style={{
              background: 'linear-gradient(145deg, #14b8a6, #0d9488)',
              boxShadow: '8px 8px 16px #bec3c9, -8px -8px 16px #ffffff'
            }}
          >
            <Mail className="w-10 h-10 text-white" />
          </div>
          <h2 className="text-xl font-bold text-gray-700 mb-3">Cek Email Anda</h2>
          <p className="text-gray-500 mb-6">
            Kami telah mengirim link verifikasi ke <strong>{email}</strong>. 
            Klik link tersebut untuk mengaktifkan akun Anda.
          </p>
          <button
            onClick={() => {
              setEmailSent(false);
              setEmail('');
              setPassword('');
              setIsSignUp(false);
            }}
            className="text-teal-500 font-medium hover:underline"
          >
            Kembali ke Login
          </button>
        </div>
      </div>
    );
  }

  return (
    <div className="min-h-screen flex items-center justify-center p-4" style={{ background: '#e0e5ec' }}>
      <div 
        className="w-full max-w-sm p-8 rounded-3xl"
        style={{
          background: '#e0e5ec',
          boxShadow: '20px 20px 60px #bec3c9, -20px -20px 60px #ffffff'
        }}
      >
        {/* Logo */}
        <div className="flex justify-center mb-6">
          <div 
            className="w-20 h-20 rounded-full overflow-hidden flex items-center justify-center"
            style={{
              background: 'linear-gradient(145deg, #f8f8f8, #ffffff)',
              boxShadow: '8px 8px 16px #bec3c9, -8px -8px 16px #ffffff'
            }}
          >
            <img src="/images/logo.png" alt="Logo" className="w-full h-full object-cover" />
          </div>
        </div>

        {/* Title */}
        <div className="text-center mb-8">
          <h1 className="text-2xl font-bold text-gray-700">ESP32 Dashboard</h1>
          <p className="text-gray-500 text-sm tracking-wider">Control System</p>
        </div>

        {/* Form */}
        {isForgotPassword ? (
          <form onSubmit={handleForgotPassword} className="space-y-5">
            {/* Email Input */}
            <div 
              className="flex items-center gap-3 px-5 py-4 rounded-full"
              style={{
                background: '#e0e5ec',
                boxShadow: 'inset 8px 8px 16px #bec3c9, inset -8px -8px 16px #ffffff'
              }}
            >
              <User className="w-5 h-5 text-gray-400" />
              <input
                type="email"
                placeholder="email"
                value={email}
                onChange={(e) => setEmail(e.target.value)}
                required
                className="flex-1 bg-transparent border-none outline-none text-gray-600 placeholder-gray-400"
              />
            </div>

            {/* Submit Button */}
            <button
              type="submit"
              disabled={loading}
              className="w-full py-4 rounded-full font-semibold text-white transition-all duration-300 hover:opacity-90 disabled:opacity-50"
              style={{
                background: 'linear-gradient(145deg, #5eead4, #14b8a6)',
                boxShadow: '8px 8px 16px #bec3c9, -8px -8px 16px #ffffff'
              }}
            >
              {loading ? 'Loading...' : 'Kirim Link Reset'}
            </button>

            {/* Back Button */}
            <button
              type="button"
              onClick={() => {
                setIsForgotPassword(false);
                setEmail('');
              }}
              className="w-full flex items-center justify-center gap-2 py-3 text-gray-500 hover:text-gray-700 transition-colors"
            >
              <ArrowLeft className="w-4 h-4" />
              Kembali ke Login
            </button>
          </form>
        ) : (
          <div className="space-y-5">
            <form onSubmit={isSignUp ? handleSignUp : handleSignIn} className="space-y-5">
              {/* Email Input */}
              <div 
                className="flex items-center gap-3 px-5 py-4 rounded-full"
                style={{
                  background: '#e0e5ec',
                  boxShadow: 'inset 8px 8px 16px #bec3c9, inset -8px -8px 16px #ffffff'
                }}
              >
                <User className="w-5 h-5 text-gray-400" />
                <input
                  type="email"
                  placeholder="email"
                  value={email}
                  onChange={(e) => setEmail(e.target.value)}
                  required
                  className="flex-1 bg-transparent border-none outline-none text-gray-600 placeholder-gray-400"
                />
              </div>

              {/* Password Input */}
              <div 
                className="flex items-center gap-3 px-5 py-4 rounded-full"
                style={{
                  background: '#e0e5ec',
                  boxShadow: 'inset 8px 8px 16px #bec3c9, inset -8px -8px 16px #ffffff'
                }}
              >
                <Lock className="w-5 h-5 text-gray-400" />
                <input
                  type="password"
                  placeholder="password"
                  value={password}
                  onChange={(e) => setPassword(e.target.value)}
                  required
                  minLength={6}
                  className="flex-1 bg-transparent border-none outline-none text-gray-600 placeholder-gray-400"
                />
              </div>

              {/* Submit Button */}
              <button
                type="submit"
                disabled={loading}
                className="w-full py-4 rounded-full font-semibold text-white transition-all duration-300 hover:opacity-90 disabled:opacity-50"
                style={{
                  background: 'linear-gradient(145deg, #5eead4, #14b8a6)',
                  boxShadow: '8px 8px 16px #bec3c9, -8px -8px 16px #ffffff'
                }}
              >
                {loading ? 'Loading...' : isSignUp ? 'Daftar' : 'Login'}
              </button>
            </form>

            {/* Divider */}
            <div className="flex items-center gap-3">
              <div className="flex-1 h-px bg-gray-300"></div>
              <span className="text-gray-400 text-sm">atau</span>
              <div className="flex-1 h-px bg-gray-300"></div>
            </div>

            {/* Google Sign In Button */}
            <button
              type="button"
              onClick={handleGoogleSignIn}
              disabled={loading}
              className="w-full py-4 rounded-full font-semibold text-gray-600 transition-all duration-300 hover:opacity-90 disabled:opacity-50 flex items-center justify-center gap-3"
              style={{
                background: '#e0e5ec',
                boxShadow: '8px 8px 16px #bec3c9, -8px -8px 16px #ffffff'
              }}
            >
              <svg className="w-5 h-5" viewBox="0 0 24 24">
                <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
                <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
                <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
                <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
              </svg>
              Lanjut dengan Google
            </button>
          </div>
        )}

        {/* Footer Links */}
        {!isForgotPassword && (
          <div className="text-center mt-6 text-sm text-gray-500">
            <button 
              onClick={() => setIsForgotPassword(true)}
              className="text-teal-500 font-medium hover:underline"
            >
              Lupa password?
            </button>
            <span className="mx-2">•</span>
            <button 
              onClick={() => setIsSignUp(!isSignUp)}
              className="text-teal-500 font-medium hover:underline"
            >
              {isSignUp ? 'Login' : 'Daftar'}
            </button>
          </div>
        )}
      </div>
    </div>
  );
};

export default Auth;
